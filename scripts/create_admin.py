import paramiko, sys

client = paramiko.SSHClient()
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect('89.116.228.186', username='root', password='Rah@sri58961', timeout=15)

def run(cmd, timeout=30):
    sys.stdout.write(f'\n>> {cmd[:120]}\n'); sys.stdout.flush()
    _, out, err = client.exec_command(cmd, timeout=timeout)
    r = (out.read() + err.read()).decode('utf-8', errors='replace').strip()
    if r: sys.stdout.write(r + '\n'); sys.stdout.flush()
    return r

# Write SQL to a temp file on the server, then run it — avoids all escaping issues
BCRYPT = r'$2b$10$X8.QEECmdyqUBeDwaqDNTuxw88zXWLkXlwmQJkiX7P23KxdVG1sxe'
sql = f"""DO $$
DECLARE
  uid uuid;
  oid uuid := gen_random_uuid();
BEGIN
  INSERT INTO "User" (id, email, password, "providerName", "providerId", timezone, activated, "isSuperAdmin", "createdAt", "updatedAt", "lastReadNotifications", "lastOnline")
  VALUES (gen_random_uuid(), 'admin@postlaa.com', '{BCRYPT}', 'LOCAL', '', 0, true, true, now(), now(), now(), now())
  ON CONFLICT (email) DO UPDATE SET "isSuperAdmin" = true, password = '{BCRYPT}', activated = true;
  SELECT id INTO uid FROM "User" WHERE email = 'admin@postlaa.com';
  INSERT INTO "Organization" (id, name, "createdAt", "updatedAt") VALUES (oid, 'Postlaa Admin', now(), now());
  INSERT INTO "UserOrganization" (id, "userId", "organizationId", role, "createdAt", "updatedAt")
  VALUES (gen_random_uuid(), uid, oid, 'SUPERADMIN', now(), now())
  ON CONFLICT DO NOTHING;
  RAISE NOTICE 'Admin created/updated with org: %', oid;
END $$;
"""

# Upload SQL file via SFTP
sftp = client.open_sftp()
with sftp.open('/tmp/create_admin.sql', 'w') as f:
    f.write(sql)
sftp.close()
sys.stdout.write('  SQL file uploaded to /tmp/create_admin.sql\n'); sys.stdout.flush()

print('\n=== Create SuperAdmin ===')
run('docker exec -i postlaa-postgres psql -U postiz -d postiz_db < /tmp/create_admin.sql 2>&1')

print('\n=== Verify admin created ===')
run('docker exec postlaa-postgres psql -U postiz -d postiz_db -c "SELECT email, activated, \\\"isSuperAdmin\\\" FROM \\\"User\\\" WHERE email = \'admin@postlaa.com\';" 2>&1')

print('\n=== Quick backend connectivity test ===')
# Try to reach the backend from within the same container
run('docker exec postlaa sh -c "cat /proc/net/tcp6 | grep 0BB8 | head -2"')

# Test via nginx (port 5000)
run('docker exec postlaa sh -c "cat /proc/net/tcp | grep 1388 | head -2 || cat /proc/net/tcp6 | grep 1388 | head -2"')

print('\n=== All container status ===')
run('cd /opt/postlaa && docker compose ps 2>&1')

client.close()
sys.stdout.write('\n=== DONE ===\n'); sys.stdout.flush()
